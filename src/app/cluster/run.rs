...
impl Run<ScyllaEvent, ScyllaHandle> for Cluster {
            ...
            match event {
                ...
                ClusterEvent::AddNode(address) => {
                    ...
                    if self.nodes.contains_key(&address) {
                        let event = ScyllaEvent::Result(ScyllaSocketMsg::Scylla(Err(Topology::AddNode(address))));
                        ...
                        Ok(mut cqlconn) => {
                            ...
                            if let (Some(dc), Some(tokens)) = (cqlconn.take_dc(), cqlconn.take_tokens()) {
                                ...
                                let mut node = NodeBuilder::new()
                                    ...
                                    .send_buffer_size(self.send_buffer_size)
                                    .authenticator(self.authenticator.clone())
                                    .build(self.service.spawn(format!("Node_{}", address)));
                ...
                ClusterEvent::RemoveNode(address) => {
                    ...
                    if let Some(mut node_info) = self.nodes.remove(&address) {
                        ...
                        let event = ScyllaEvent::Result(ScyllaSocketMsg::Scylla(Err(Topology::RemoveNode(address))));
                        supervisor.send(event).ok();
                    };
                }
                ClusterEvent::RegisterReporters(microservice, reporters_handles) => {
                    ...
                    if let Some(address) = microservice.name.split("_").nth(1).and_then(|s| s.parse().ok()) {
                        // update service
                        self.service.update_microservice(microservice);
                        ...
                        info!("Sending result event to scylla!");
                        let event = ScyllaEvent::Result(ScyllaSocketMsg::Scylla(Ok(Topology::AddNode(address))));
                        supervisor.send(event).ok();
                    } else {
                        error!("Failed to parse node address!");
                    }
                }
                ClusterEvent::BuildRing(uniform_rf) => {
                    ...
                    if !microservices.any(|ms| ms.is_starting()) && self.should_build {
                        ...
                        // reply to scylla/dashboard
                        let event = ScyllaEvent::Result(ScyllaSocketMsg::Scylla(Ok(Topology::BuildRing(uniform_rf))));
                        supervisor.send(event).ok();
                    } else {
                        // reply to scylla/dashboard
                        let event = ScyllaEvent::Result(ScyllaSocketMsg::Scylla(Err(Topology::BuildRing(uniform_rf))));
                        supervisor.send(event).ok();
                    }
                }
